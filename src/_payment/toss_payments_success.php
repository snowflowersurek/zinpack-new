...
if($paytype == '카드'){
	...
	$useEscrow = ($responseJson->useEscrow==true)?"1":"0";	// 구매자가 에스크로 사용요청 시 true
	$nointyn = '0';
	$lgd_resp = "입금대기";
}else{
	$paytype_code = 'OTHER';
...
<form method="post" name ="LGD_PAYINFO" id="LGD_PAYINFO" action="<?=$row['res_url']?>">
...
<input type="hidden" name="LGD_ESCROW"             id="LGD_ESCROW"	value="<?= $useEscrow ?>">
</form>
<div>결제 완료</div>
<script>
	/*
