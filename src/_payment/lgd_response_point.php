    ...
    if ($xpay->TX()) {
        ...
        if( "0000" == $xpay->Response_Code() ) {		
			...
			if( !$isDBOK ) {
                ...
                if( "0000" == $xpay->Response_Code() ) {
					alert("자동취소가 정상적으로 완료 되었습니다.","http://$return_domain/bbs/m/all_point_charge.php?type=$payment[type]&ep=$payment[store]&gp=$payment[group]"); 
                }else{
					alert("자동취소가 정상적으로 처리되지 않았습니다.","http://$return_domain/bbs/m/all_point_charge.php?type=$payment[type]&ep=$payment[store]&gp=$payment[group]");  
                }
          	}else{
          	...
          	//최종결제요청 결과 실패 DB처리
         	alert("결제요청이 실패하였습니다.","http://$return_domain/bbs/m/all_point_charge.php?type=$payment[type]&ep=$payment[store]&gp=$payment[group]");          	            
        }
    }else {
        //2)API 요청실패 화면처리
