...
if ($json->status) {
	...
	*/
	...
	if($status == "DONE") {
		/* 입금이 이루어졌다면, orderId로 결제조회를 해서 '현금영수증 URL' 정보를 얻어온다.
		...
		*/
		...
		if($payment_type == "point"){
			...
			$sql = "update iw_member set
					mb_point = mb_point + ?
					where ep_code = ? and mb_code = ?
					";
            $stmt = mysqli_prepare($db_conn, $sql);
            mysqli_stmt_bind_param($stmt, "dss", $chk_castamount, $ep_code, $mb_code);
            mysqli_stmt_execute($stmt);
            mysqli_stmt_close($stmt);
			
			$sql = " select mb_point from iw_member where ep_code = ? and mb_code = ?";
            $stmt = mysqli_prepare($db_conn, $sql);
            mysqli_stmt_bind_param($stmt, "ss", $ep_code, $mb_code);
            mysqli_stmt_execute($stmt);
            $result = mysqli_stmt_get_result($stmt);
            $row = mysqli_fetch_assoc($result);
            mysqli_stmt_close($stmt);
			...
			$sql = "update iw_point set
					...
					";
            $stmt = mysqli_prepare($db_conn, $sql);
            mysqli_stmt_bind_param($stmt, "dsss", $mb_point, $ep_code, $mb_code, $orderid);
            mysqli_stmt_execute($stmt);
            mysqli_stmt_close($stmt);
		}else{
        ...
        $row2 = null;
        $sql = "select mb_code from iw_enterprise where ep_code = ?";
        $stmt = mysqli_prepare($db_conn, $sql);
        mysqli_stmt_bind_param($stmt, "s", $ep_code);
        mysqli_stmt_execute($stmt);
        $result = mysqli_stmt_get_result($stmt);
		...
		//판매자에게 메일
		$sql = "select DISTINCT(seller_mb_code) from iw_shop_order_sub where ep_code = ? and mb_code = ? and sr_code = ?";
        $stmt = mysqli_prepare($db_conn, $sql);
        mysqli_stmt_bind_param($stmt, "sss", $ep_code, $mb_code, $orderid);
        mysqli_stmt_execute($stmt);
        $result = mysqli_stmt_get_result($stmt);
		...
		while($row = mysqli_fetch_assoc($result)){
            ...
            $row2 = null;
            $sql2 = "select mb_mail from iw_member where ep_code = ? and mb_code = ?";
            $stmt2 = mysqli_prepare($db_conn, $sql2);
            mysqli_stmt_bind_param($stmt2, "ss", $ep_code, $seller_mb_code);
