		...
		<div class="row">
			<div class="col-xs-12">
				<div class="table-header">
					2015 골든티어워드(GCA) "블렌딩티콘테스트"
				</div>
				<div class="space-4"></div>

			<!-- PAGE CONTENT BEGINS -->
				<form class="form-horizontal" id="mail_form" name="mail_form" action="coffeentea_3_ok.php?type=<?=$iw[type]?>&ep=<?=$iw[store]?>&gp=<?=$iw[group]?>" method="post">
					...
					<div class="form-group">
						...
						<div class="col-sm-8">
							<select class="col-xs-12 col-sm-12" name="mail_01">
								...
								<option value="녹차블렌딩 부문">녹차블렌딩 부문</option>
								<option value="홍차블렌딩 부문">홍차블렌딩 부문</option>
							</select>
						</div>
					</div>
					<div class="space-4"></div>
					...
					<div class="form-group">
						...
						<div class="col-sm-8">
							<input type="text" class="col-xs-12 col-sm-12" name="mail_12">
						</div>
					</div>
					<div class="space-4"></div>
					...
					<div class="form-group">
						...
						<div class="col-sm-8">
							<textarea name='mail_13' class='col-xs-12 col-sm-12' style='height:100px;resize:none;'></textarea>
						</div>
					</div>
...
<script type="text/javascript">
	function check_form() {
		...
		if (mail_form.mail_12.value.search(/^((\w|[\-\.])+)@((\w|[\-\.][^(\.)\1])+)\.([A-Za-z]+)$/)== -1){
			alert("잘못된 이메일 주소입니다.");
			mail_form.mail_12.focus();
			return;
		}
