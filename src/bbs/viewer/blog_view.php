...
$sql = "select * from $iw[book_blog_table] where ep_code = '$iw[store]' and gp_code = '$iw[group]' and bd_code = '$bd_code' and bg_no = '$bg_no'";
$row = sql_fetch($sql);
...
<div class="book_main_wrap" style="background-color:<?=$bn_color?>;">
	<div class="wrap">
		<div class="top_back" style="border-bottom:2px solid <?=$bn_font?>;">
			<a href="<?=$iw['viewer_path']?>/blog_main.php?type=<?=$iw[type]?>&ep=<?=$iw[store]?>&gp=<?=$iw[group]?>&idx=<?=$bd_code?>" style="color:<?=$bn_font?>;">< BACK</a>
		</div>
		<div class="pdf_viewer">
		<iframe type="text/html" width="100%" height="100%" src="pdf_viewer/viewer.php?file=<?=$upload_path."/".$bn_file?>#page=<?=$bg_page?>" frameborder="0">
		</iframe>
		</div>
	</div>
</div>
