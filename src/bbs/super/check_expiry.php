...
if($chk_date=="" || $chk_date != $chkdate){
	...
	while($row = sql_fetch_array($result)){
		...
		$epcode = $row['ep_code'];
		$memsql = "SELECT * FROM $iw[member_table] WHERE ep_code = '".$epcode."' AND mb_level = 9";
		...
		if($diff_days < 32 && $diff_days > 0){
			if($sdate==""){
				...
				if($last_diff > (3600 * 24 * 7)){
					...
					echo "쫌 더 있다 다시 보내야지 ~ " . $last_date;
				}
			}
		}else if($diff_days < -7 && $diff_days > -15){	// 유효기간이 지난지 일주일 ~ 보름 사이라면
			echo $diff_days . "일주일 ~ 보름 전<br>";
			// 최종 공지 이메일을 보낸다.
			if($last_diff > (3600 * 24 * 7)){
				$usql = "UPDATE $iw[enterprise_table] SET expiry_email = CONCAT(expiry_email, '".$sent_date."') WHERE ep_no = ".$row['ep_no'];
				sql_query($usql);
			...
			echo $diff_days . "보름 ~ 한달 전<br>";
			// 삭제 공지 후, db 삭제 ???
			$usql = "UPDATE $iw[enterprise_table] SET avail_type=4, expiry_email = CONCAT(expiry_email, '".$sent_date."') WHERE ep_no = ".$row['ep_no'];
			echo $usql."<br>";
			sql_query($usql);

			$title = '<strong style="color:#df0100;">ZINPACK 이용 연장 가능일</strong>이 다 되었습니다.';
			$subject = "그동안 이용해 주셔서 감사드립니다. 귀사의 사이트가 자동으로 삭제됨을 알려드립니다. 궁금하신 점이 있으시면 연락바랍니다.";
			expiry_notice_email($to, $subject, $epcode, $title);
		}else{
			echo "이도저도 아니면 뭐야 ?".$diff_days."<br>";
		}
		$chkcnt++;
	}
	set_cookie("expiry_chk_date", $chkdate, time()+3600*24);
	echo "check count : ".$chkcnt;
}else{
	echo "오늘 체크했었구나~~~";
	set_cookie("expiry_chk_date", "", time()+3600*24);
}
...
function expiry_notice_email($to, $subject, $ep_code, $title){
	...
	-	ep_corporate (업체명)
	-	ep_expiry_date (만료일)
	-	ep_expiry_date + 15일 (만료일이 지난 15일까지가 연장 가능일)
	-	ep_expiry_date + 16일 (삭제일)
	-	https://www.info-way.co.kr/bbs/m/all_login.php?type=main&ep=ep1822322763609cab5915c89&gp=all
	*/
