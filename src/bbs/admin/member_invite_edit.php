...
$sql = "select * from $iw[group_invite_table] where ep_code = '$iw[store]' and gp_code = '$iw[group]' and gi_no = '$gi_no'";
$row = sql_fetch($sql);
if (!$row["gi_no"]) alert("잘못된 접근입니다!","");

$gi_tel= explode("-", $row["gi_tel"]);
...
<div class="page-content">
	...
	<div class="container">
		<div class="row">
			<div class="col-xs-12">
			<!-- PAGE CONTENT BEGINS -->
				<form class="form-horizontal" id="gi_form" name="gi_form" action="<?=$iw['admin_path']?>/member_invite_edit_ok.php?type=<?=$iw[type]?>&ep=<?=$iw[store]?>&gp=<?=$iw[group]?>" method="post">
				<input type="hidden" name="gi_no" value="<?=$gi_no?>" />
					...
					<div class="form-group">
						...
						<div class="col-sm-11">
							<input type="text" placeholder="입력" class="col-xs-12 col-sm-8" name="gi_message" maxlength="150" value="<?=$row[gi_message]?>">
						</div>
					</div>
					<div class="space-4"></div>
					...
					<div class="form-group">
						...
						<div class="col-sm-11">
							<input type="text" placeholder="입력" name="gi_datetime_end_1" maxlength="4" value="<?=date("Y", strtotime($gi_datetime_end))?>">년
							<input type="text" placeholder="입력" name="gi_datetime_end_2" maxlength="2" value="<?=date("m", strtotime($gi_datetime_end))?>">월
							<input type="text" placeholder="입력" name="gi_datetime_end_3" maxlength="2" value="<?=date("d", strtotime($gi_datetime_end))?>">일
						</div>
					</div>
					<div class="clearfix form-actions">
						<div class="col-md-offset-3 col-md-9">
							...
							<button class="btn btn-danger" type="button" onclick="javascript:delete_invite('<?=$iw[type]?>', '<?=$iw[store]?>', '<?=$iw[group]?>', '<?=$gi_no?>');">
								<i class="fa fa-check"></i>
								삭제
							</button>
							<button class="btn btn-default" type="button" onclick="location='<?=$iw['admin_path']?>/member_invite_list.php?type=<?=$iw[type]?>&ep=<?=$iw[store]?>&gp=<?=$iw[group]?>'">
...
<script type="text/javascript">
	...
	function delete_invite(type ,ep, gp, idx)
    {
		if (confirm('초대정보를 삭제하시겠습니까?')) {
			location.href="member_invite_delete.php?type="+type+"&ep="+ep+"&gp="+gp+"&idx="+idx;
		}
    }
</script>
