						...
						<section class="content-box">
							...
							<div class="table-set-mobile dataTable-wrapper">
									...
									<div class="col-sm-6">
										<div class="dataTable-option-right">
											<?
												...
												if($search =="a"){
													$search_sql = "and seller_mb_code like '%$searchs%'";
												}else if($search =="b"){
													$search_sql = "and bd_code like '%$searchs%'";
												}else if($search =="c"){
													$search_sql = "and mb_code like '%$searchs%'";
												}else if($search =="d"){
													$search_sql = "and bs_subject like '%$searchs%'";
												}
											?>
											...
											<label>검색: <select name="search">
												...
												<option value="b" <?if($search == "b"){?>selected="selected"<?}?>>이북코드</option>
												<option value="c" <?if($search == "c"){?>selected="selected"<?}?>>후원회원코드</option>
												<option value="d" <?if($search == "d"){?>selected="selected"<?}?>>이북명</option>
								...
								<table class="table table-striped table-bordered table-hover dataTable">
									...
									<tbody>
									<?
										$sql = "select * from $iw[book_support_table] where ep_code = '$iw[store]' $search_sql";
										...
										$sql = "select * from $iw[book_support_table] where ep_code = '$iw[store]' $search_sql order by bs_no desc limit $start_line, $end_line";
										$result = sql_query($sql);

										$i=0;
										while($row = @sql_fetch_array($result)){
											$bs_subject = stripslashes($row["bs_subject"]);
											$bd_code = $row["bd_code"];
											$mb_code = $row["mb_code"];
									...
									?>
										<tr>
											<td data-title="판매자코드"><?=$seller_mb_code?></td>
											<td data-title="이북코드"><?=$bd_code?></td>
											<td data-title="이북명"><?=$bs_subject?></td>
											<td data-title="후원회원"><?=$mb_nick?></td>
											<td data-title="후원금액"><?=number_format($bs_price)?> Point</td>
											<td data-title="판매일"><?=$bs_datetime?></td>
										</tr>
									<?
										$i++;
										}
