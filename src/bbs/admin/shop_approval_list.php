						...
						<section class="content-box">
							...
							<div class="table-set-mobile dataTable-wrapper">
									...
									<div class="col-sm-6">
										<div class="dataTable-option-right">
											...
											<label>검색: <select name="search">
												<option value="a" <?if($search == "a"){?>selected="selected"<?}?>>상품코드</option>
								...
								<table class="table table-striped table-bordered table-hover dataTable">
									...
									<tbody>
									<?
										$sql = "select * from $iw[shop_data_table] where ep_code = '$iw[store]' and gp_code = '$iw[group]' and sd_display = 2 $search_sql";
										...
										$sql = "select * from $iw[shop_data_table] where ep_code = '$iw[store]' and gp_code = '$iw[group]' and sd_display = 2 $search_sql order by sd_no desc limit $start_line, $end_line";
										$result = sql_query($sql);
										...
										while($row = @sql_fetch_array($result)){
											...
											$sd_price = number_format($row["sd_price"]);
											$sd_sale = number_format($row["sd_sale"]);
											$mb_code = $row["mb_code"];

											$rowcnt = sql_fetch(" select count(*) as cnt from $iw[shop_option_table] where ep_code = '$iw[store]' and gp_code = '$iw[group]' and sd_code = '$sd_code'");
									...
									?>
										<tr>
											<td data-title="상품코드"><a href="<?=$iw['admin_path']?>/shop_approval_view.php?type=<?=$iw[type]?>&ep=<?=$iw[store]?>&gp=<?=$iw[group]?>&idx=<?=$sd_code?>"><?=$sd_code?></a></td>
											...
											<td data-title="승인"><a href="javascript:approval_edit('<?=$iw[type]?>','<?=$iw[store]?>','<?=$iw[group]?>','<?=$sd_code?>','1');"><span class="label label-sm label-success">승인하기</span></a></td>
										</tr>
									<?
										$i++;
										}
										if($i==0) echo "<tr><td colspan='7' align='center'>승인할 상품이 없습니다.</td></tr>";
									?>
									</tbody>
								</table>
								<div class="row">
...
<script type="text/javascript">
	function approval_edit(type,ep,gp,no,dis) {
		if (confirm("상품등록을 승인하시겠습니까?")) {
			location.href="shop_approval_ok.php?type="+type+"&ep="+ep+"&gp="+gp+"&idx="+no+"&dis="+dis;
		}
	}
</script>
<?
