						...
						<section class="content-box">
							...
							<div class="table-set-mobile dataTable-wrapper">
								...
								<table class="table table-striped table-bordered table-hover dataTable">
									...
									<tbody>
									<?php
										$sql = "select * from $iw[lgd_cancel_table] where ep_code = '$iw[store]' and state_sort = '$iw[type]' $search_sql";
										...
										$sql = "select * from $iw[lgd_cancel_table] where ep_code = '$iw[store]' and state_sort = '$iw[type]' $search_sql order by lgdc_no desc limit $start_line, $end_line";
										$result = sql_query($sql);
										...
										while($row = @sql_fetch_array($result)){
											...
											$lgdc_datetime = date( "Y-m-d H:i:s", strtotime($row["lgdc_datetime"]) );

											$sqls = "select * from $iw[lgd_table] where ep_code = '$iw[store]' and state_sort = '$iw[type]' and lgd_tid = '$lgdc_tid' and lgd_display = 2";
											$rows = sql_fetch($sqls);
											$lgd_amount = number_format($rows["lgd_amount"]);
									...
									?>
										<tr>
											...
											<td data-title="취소금액"><?=$lgd_amount?> 원</td>
											<td data-title="승인번호"><?=$lgd_financeauthnum?></td>
											<td data-title="승인결과"><?=$lgdc_respmsg?></td>
											<td data-title="승인일시"><?=$lgdc_datetime?></td>
											<td data-title="결제방식">
									...
									<?php
										...
										if($i==0) echo "<tr><td colspan='7' align='center'>LGU+취소내역이 없습니다.</td></tr>";
									?>
									</tbody>
								</table>
								<div class="row">
