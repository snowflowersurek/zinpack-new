...
$row = sql_fetch(" select * from $iw[category_table] where cg_code = '$_GET[menu]' and state_sort = '$iw[type]' and ep_code = '$iw[store]' and gp_code = '$iw[group]'");
...
<div class="page-content">
	<div class="container">
		<div class="row">
			<div class="col-xs-12">
			<!-- PAGE CONTENT BEGINS -->
				<form class="form-horizontal" id="cg_form" name="cg_form" action="<?=$iw['admin_path']?>/category_edit_ok.php?type=<?=$iw[type]?>&ep=<?=$iw[store]?>&gp=<?=$iw[group]?>" method="post">
				<input type="hidden" name="cg_code" value="<?=$row['cg_code']?>" />
					<div class="form-group">
						<label class="col-sm-2 control-label">분류명</label>
						<div class="col-sm-8">
							<input type="text" placeholder="입력" class="col-xs-12 col-sm-12" name="cg_name" value="<?=$row['cg_name']?>">
						</div>
					</div>
