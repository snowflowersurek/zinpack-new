						...
						<section class="content-box">
							...
							<div class="table-set-mobile dataTable-wrapper">
								...
								<table class="table table-striped table-bordered table-hover dataTable">
									...
									<tbody>
									<?php
										$sql = "select * from $iw[doc_data_table] where ep_code = '$iw[store]' and gp_code = '$iw[group]' and dd_display = 2 $search_sql";
										...
										$sql = "select * from $iw[doc_data_table] where ep_code = '$iw[store]' and gp_code = '$iw[group]' and dd_display = 2 $search_sql order by dd_no desc limit $start_line, $end_line";
										$result = sql_query($sql);
									...
									?>
										<tr>
											<td data-title="컨텐츠코드"><a href="<?=$iw['admin_path']?>/doc_approval_view.php?type=<?=$iw[type]?>&ep=<?=$iw[store]?>&gp=<?=$iw[group]?>&idx=<?=$dd_code?>"><?=$dd_code?></a></td>
											...
											<td data-title="승인"><a href="javascript:approval_edit('<?=$iw[type]?>','<?=$iw[store]?>','<?=$iw[group]?>','<?=$dd_code?>','1');"><span class="label label-sm label-success">승인하기</span></a></td>
										</tr>
									<?php
										$i++;
										}
										if($i==0) echo "<tr><td colspan='7' align='center'>승인할 컨텐츠가 없습니다.</td></tr>";
									?>
									</tbody>
								</table>
								<div class="row">
...
<script type="text/javascript">
	function approval_edit(type,ep,gp,no,dis) {
		if (confirm("컨텐츠등록을 승인하시겠습니까?")) {
			location.href="doc_approval_ok.php?type="+type+"&ep="+ep+"&gp="+gp+"&idx="+no+"&dis="+dis;
		}
	}
</script>
<?php
