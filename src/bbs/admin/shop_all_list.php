						...
						<section class="content-box">
							...
							<div class="table-set-mobile dataTable-wrapper">
									...
									<div class="col-sm-6">
										<div class="dataTable-option-right">
											...
											<label>검색: <select name="search">
												...
												<option value="c" <?if($search == "c"){?>selected="selected"<?}?>>상품명</option>
								...
								<table class="table table-striped table-bordered table-hover dataTable">
									...
									<tbody>
									<?
										$sql = "select * from $iw[shop_data_table] where ep_code = '$iw[store]' $search_sql";
										...
										$sql = "select * from $iw[shop_data_table] where ep_code = '$iw[store]' $search_sql order by sd_no desc limit $start_line, $end_line";
										$result = sql_query($sql);
									...
									?>
										<tr>
											<td data-title="상품코드"><a href="<?=$iw['admin_path']?>/shop_all_view.php?type=<?=$iw[type]?>&ep=<?=$iw[store]?>&gp=<?=$iw[group]?>&idx=<?=$sd_code?>"><?=$sd_code?></a></td>
									...
									<?
										...
										if($i==0) echo "<tr><td colspan='5' align='center'>검색된 상품이 없습니다.</td></tr>";
									?>
									</tbody>
								</table>
								<div class="row">
