...
<div class="page-content">
	<div class="container">
		<div class="row">
			<div class="col-xs-12">
			<!-- PAGE CONTENT BEGINS -->
				<form class="form-horizontal" id="bm_form" name="bm_form" action="<?=$iw['admin_path']?>/media/media_zoom_edit_ok.php?type=<?=$iw[type]?>&ep=<?=$iw[store]?>&gp=<?=$iw[group]?>" method="post" enctype="multipart/form-data">
				...
				<input type="hidden" name="bmd_image_old" value="<?=$bmd_image?>" />
					<div class="form-group">
						...
						<div class="col-sm-8">
							<span class="help-block col-xs-12">무한확대</span>
						</div>
					</div>
					...
					<div class="form-group">
						...
						<div class="col-sm-8">
							...
							<span class="help-block col-xs-12">
								1페이지만 등록
								<?php if($bmd_image){?><br/><a href="<?=$iw["path"].$upload_path."/".$bmd_image?>" target="_blank">PDF 보기</a><?php }?>
							</span>
						</div>
					</div>
...
<script type="text/javascript">
	function check_form() {
		...
		if (bm_form.bmd_image.value && !bm_form.bmd_image.value.match(/(.pdf|.PDF)/)) { 
	...
	function page_delete(type,ep,gp,idx,no) { 
		if (confirm('삭제하시겠습니까?')) {
			location.href="media_zoom_delete.php?type="+type+"&ep="+ep+"&gp="+gp+"&idx="+idx+"&no="+no;
		}
	}
</script>
