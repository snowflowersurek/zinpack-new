...
<div class="page-content">
	<div class="container">
		<div class="row">
			<div class="col-xs-12">
			<!-- PAGE CONTENT BEGINS -->
				<form class="form-horizontal" id="bm_form" name="bm_form" action="<?=$iw['admin_path']?>/media/media_text_edit_ok.php?type=<?=$iw[type]?>&ep=<?=$iw[store]?>&gp=<?=$iw[group]?>" method="post" enctype="multipart/form-data">
				...
				<input type="hidden" name="bm_image_old" value="<?=$bm_image?>" />
					<div class="form-group">
						...
						<div class="col-sm-8">
							<span class="help-block col-xs-12">기사</span>
						</div>
					</div>
					...
					<div class="form-group">
						...
						<div class="col-sm-8">
							<input type="text" placeholder="입력" class="col-xs-12 col-sm-12" name="bmd_image" value="<?=$bmd_image?>">
							<span class="help-block col-xs-12">
								최대 30Byte 이하
							</span>
						</div>
					...
					<div class="form-group">
						...
						<div class="col-sm-8">
							<textarea name="bmd_content" class="col-xs-12 col-sm-12" style="height:300px;resize:none;"><?=$bmd_content?></textarea>
						</div>
					</div>
...
<script type="text/javascript">
	function check_form() {
		...
		if (bm_form.bmd_image.value == ""){
			alert("기자를 입력하여 주십시오.");
			bm_form.bmd_image.focus();
			return;
		}

		ls_str = bm_form.bmd_image.value; // 이벤트가 일어난 컨트롤의 value 값
		...
		if (bm_form.bmd_content.value == ""){
			alert("기사내용을 입력하여 주십시오.");
			bm_form.bmd_content.focus();
			return;
		}
		bm_form.submit();
	}
	...
	function page_delete(type,ep,gp,idx,no) { 
		if (confirm('삭제하시겠습니까?')) {
			location.href="media_text_delete.php?type="+type+"&ep="+ep+"&gp="+gp+"&idx="+idx+"&no="+no;
		}
	}
</script>
