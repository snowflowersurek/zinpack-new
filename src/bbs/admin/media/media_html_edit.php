...
<div class="page-content">
	<div class="container">
		<div class="row">
			<div class="col-xs-12">
			<!-- PAGE CONTENT BEGINS -->
				<form class="form-horizontal" id="bm_form" name="bm_form" action="<?=$iw['admin_path']?>/media/media_html_edit_ok.php?type=<?=$iw[type]?>&ep=<?=$iw[store]?>&gp=<?=$iw[group]?>" method="post" enctype="multipart/form-data">
				...
				<input type="hidden" name="bm_image_old" value="<?=$bm_image?>" />
					...
					<div class="form-group">
						...
						<div class="col-sm-8">
							<textarea name="contents1" style="height:300px;width:100%;" editor="webnote" tools="deny:strikethrough,superscript,subscript,indent,outdent,insertfirstparagraph,insertlastparagraph,images,textbox,note,table,sourcebox,emoticon,special_chars,fullscreen"><?=$bmd_content?></textarea>
						</div>
					</div>
...
<script type="text/javascript">
	...
	function page_delete(type,ep,gp,idx,no) { 
		if (confirm('삭제하시겠습니까?')) {
			location.href="media_html_delete.php?type="+type+"&ep="+ep+"&gp="+gp+"&idx="+idx+"&no="+no;
		}
	}
</script>
