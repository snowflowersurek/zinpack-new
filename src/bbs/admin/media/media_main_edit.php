...
<div class="page-content">
	<div class="container">
		<div class="row">
			<div class="col-xs-12">
			<!-- PAGE CONTENT BEGINS -->
				<form class="form-horizontal" id="bn_form" name="bn_form" action="<?=$iw['admin_path']?>/media/media_main_edit_ok.php?type=<?=$iw[type]?>&ep=<?=$iw[store]?>&gp=<?=$iw[group]?>" method="post" enctype="multipart/form-data">
				<input type="hidden" name="bd_code" value="<?=$row['bd_code']?>" />
				<input type="hidden" name="upload_path" value="<?=$upload_path?>" />
				<input type="hidden" name="bn_logo_old" value="<?=$row['bn_logo']?>" />
				<input type="hidden" name="bn_thum_old" value="<?=$row['bn_thum']?>" />
				<input type="hidden" name="bn_text_logo_old" value="<?=$row['bn_text_logo']?>" />
				<input type="hidden" name="bn_display" value="<?=$row['bn_display']?>" />
					...
					<div class="form-group">
						<label class="col-sm-2 control-label">빈타이틀 썸네일</label>
						<div class="col-sm-8">
					...
					<div class="form-group">
						<label class="col-sm-2 control-label">기사 끝부분 로고</label>
						<div class="col-sm-8">
...
<script type="text/javascript">
	function check_form() {
		<?php if($row['bn_display']!=1){?>
			...
			if (bn_form.bn_thum.value == ""){
				alert("빈타이틀 썸네일을 입력하여 주십시오.");
				bn_form.bn_thum.focus();
				return;
			}
			if (bn_form.bn_text_logo.value == ""){
				alert("기사 끝부분 로고를 입력하여 주십시오.");
		...
		if (bn_form.bn_thum.value && !bn_form.bn_thum.value.match(/(.png|.PNG|.jpg|.JPG|.jpeg|.JPEG|.gif|.GIF)/)) { 
			alert('이미지 파일만 업로드 가능합니다.');
			bn_form.bn_thum.focus();
			return;
		}
		if (bn_form.bn_text_logo.value && !bn_form.bn_text_logo.value.match(/(.png|.PNG|.jpg|.JPG|.jpeg|.JPEG|.gif|.GIF)/)) { 
			alert('이미지 파일만 업로드 가능합니다.');
			bn_form.bn_text_logo.focus();
			return;
		}
