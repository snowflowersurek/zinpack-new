...
<div class="page-content">
	<div class="container">
		<div class="row">
			<div class="col-xs-12">
			<!-- PAGE CONTENT BEGINS -->
				<form class="form-horizontal" id="bm_form" name="bm_form" action="<?=$iw['admin_path']?>/media/media_image_edit_ok.php?type=<?=$iw[type]?>&ep=<?=$iw[store]?>&gp=<?=$iw[group]?>" method="post" enctype="multipart/form-data">
				...
				<input type="hidden" name="bm_image_old" value="<?=$bm_image?>" />
					...
					<div class="form-group">
						...
						<div class="col-sm-8">
							...
							<?php
								$file_num=0;
								$sql = "select * from $iw[book_media_detail_table] where ep_code = '$iw[store]' and gp_code = '$iw[group]' and bd_code = '$bd_code' and mb_code = '$iw[member]' and bm_order = '$bm_order' order by bmd_order asc";
								...
								while($row = @sql_fetch_array($result)){
									...
									if($bmd_type == 1){
										$type_name = "이미지";
										$file_help = "이미지 - 사이즈(pixel) 1024 X 1024 이하";
										$text_help = "캡션내용";
									}else if($bmd_type == 2){
										$type_name = "유튜브 동영상";
										$file_help = "동영상 썸네일 - 사이즈(pixel) 1024 X 1024 이하";
										$text_help = "유튜브 코드 - URL 끝부분 v= 다음에 있는 코드";
									}else if($bmd_type == 3){
										$type_name = "광고 링크";
										$file_help = "광고 이미지 - 사이즈(pixel) 1024 X 1024 이하";
										$text_help = "광고 링크 URL";
									}else if($bmd_type == 4){
										$type_name = "오디오";
...
<script type="text/javascript">
	var flen = <?=$file_num?>;
	parent.document.getElementById("bookFrame").height = (parent.document.getElementById("bookFrame").height * 1)+ (200*flen);
	function add_file()
	{
		var upload_count = 20;
	...
	function page_delete(type,ep,gp,idx,no) { 
		if (confirm('삭제하시겠습니까?')) {
			location.href="media_image_delete.php?type="+type+"&ep="+ep+"&gp="+gp+"&idx="+idx+"&no="+no;
		}
	}
</script>
