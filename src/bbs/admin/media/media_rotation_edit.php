...
<div class="page-content">
	<div class="container">
		<div class="row">
			<div class="col-xs-12">
			<!-- PAGE CONTENT BEGINS -->
				<form class="form-horizontal" id="bm_form" name="bm_form" action="<?=$iw['admin_path']?>/media/media_rotation_edit_ok.php?type=<?=$iw[type]?>&ep=<?=$iw[store]?>&gp=<?=$iw[group]?>" method="post" enctype="multipart/form-data">
							...
							<?
								...
								for ($i=1; $i<count($bmd_image); $i++) {
									if($bmd_image[$i]){
										...
										<tr>
											<td>
												<input type="hidden" name="bmd_image_old[]" value="<?=$bmd_image[$i]?>" />
...
<script type="text/javascript">
	var flen = <?=$file_num?>;
	parent.document.getElementById("bookFrame").height = (parent.document.getElementById("bookFrame").height * 1)+ (81*flen);
	function add_file()
	{
		var upload_count = 24;
	...
	function page_delete(type,ep,gp,idx,no) { 
		if (confirm('삭제하시겠습니까?')) {
			location.href="media_rotation_delete.php?type="+type+"&ep="+ep+"&gp="+gp+"&idx="+idx+"&no="+no;
		}
	}
</script>
