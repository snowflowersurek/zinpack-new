...
$sql = "update $iw[shop_data_table] set
		...
		where sd_code = '$sd_code' and ep_code = '$iw[store]' and gp_code = '$iw[group]'
		";

sql_query($sql);

$sql = "update $iw[total_data_table] set
		td_display = '$sd_display'
		where td_code = '$sd_code' and ep_code = '$iw[store]' and gp_code = '$iw[group]' and state_sort = '$iw[type]'
		";
sql_query($sql);

goto_url("$iw[admin_path]/shop_exposure_list.php?type=$iw[type]&ep=$iw[store]&gp=$iw[group]");
?>