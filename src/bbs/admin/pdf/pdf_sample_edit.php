...
<div class="page-content">
	...
	<div class="container">
		<div class="row">
			<div class="col-xs-12">
			<!-- PAGE CONTENT BEGINS -->
				<form class="form-horizontal" id="bd_form" name="bd_form" action="<?=$iw['admin_path']?>/pdf/pdf_sample_edit_ok.php?type=<?=$iw[type]?>&ep=<?=$iw[store]?>&gp=<?=$iw[group]?>" method="post" enctype="multipart/form-data">
				...
				<input type="hidden" name="upload_path" value="<?=$upload_path?>" />
					...
					<div class="form-group">
						...
						<div class="col-sm-9">
							[샘플] <?=stripslashes($row["bd_subject"])?>
						</div>
					</div>
					<div class="space-4"></div>
				...
				<div id="wrap_zip" style="display:none;">
					...
					<div class="clearfix form-actions">
						<div class="col-md-offset-3 col-md-9" id="submit_btn">
							<button class="btn btn-success" type="button" onclick="javascript:check_form();">
							...
							<button class="btn btn-primary" type="button" onclick="location='<?=$iw['admin_path']?>/pdf/pdf_data_edit.php?type=<?=$iw[type]?>&ep=<?=$iw[store]?>&gp=<?=$iw[group]?>&idx=<?=$bd_code?>'">
								<i class="fa fa-undo"></i>
								원본
							</button>
						</div>
