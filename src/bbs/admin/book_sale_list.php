						...
						<section class="content-box">
							...
							<div class="table-set-mobile dataTable-wrapper">
										...
										<div class="dataTable-option-right">
											<?
												...
												if($search =="a"){
													...
													$search_sql = "and bb_subject like '%$searchs%'";
												}
											?>
											...
											<label>검색: <select name="search">
												...
												<option value="b" <?if($search == "b"){?>selected="selected"<?}?>>구매회원코드</option>
												<option value="c" <?if($search == "c"){?>selected="selected"<?}?>>이북명</option>
								...
								<table class="table table-striped table-bordered table-hover dataTable">
									...
									<tbody>
									<?
										$sql = "select * from $iw[book_buy_table] where ep_code = '$iw[store]' and gp_code = '$iw[group]' and seller_mb_code='$iw[member]' $search_sql";
										...
										$sql = "select * from $iw[book_buy_table] where ep_code = '$iw[store]' and gp_code = '$iw[group]' and seller_mb_code='$iw[member]' $search_sql order by bb_no desc limit $start_line, $end_line";
										$result = sql_query($sql);
									...
									?>
										<tr>
											<td data-title="이북코드"><?=$bd_code?></td>
											<td data-title="이북명"><?=$bb_subject?></td>
											<td data-title="구매회원"><?=$mb_nick?></td>
											<td data-title="판매(point)"><?=number_format($bb_price)?></td>
											<td data-title="적립(point)"><?=$bb_price_seller?></td>
											<td data-title="판매일"><?=$bb_datetime?></td>
										</tr>
									<?
										$i++;
										}
										if($i==0) echo "<tr><td colspan='6' align='center'>판매내역이 없습니다.</td></tr>";
									?>
									</tbody>
								</table>
								<div class="row">
