...
$sql = "select * from $iw[shop_seller_table] where ep_code = '$iw[store]' and ss_display = 0 and ss_no = '$ss_no'";
$row = sql_fetch($sql);
			...
			<!-- PAGE CONTENT BEGINS -->
				...
				<div class="clearfix form-actions">
					<div class="col-md-offset-3 col-md-9">
						<button class="btn btn-primary" type="button" onclick="javascript:approval_edit('<?=$iw[type]?>','<?=$iw[store]?>','<?=$iw[group]?>','<?=$ss_no?>','1');">
							<i class="fa fa-undo"></i>
							승인
						</button>
						<button class="btn btn-default" type="button" onclick="location='<?=$iw['admin_path']?>/shop_seller_approval_list.php?type=<?=$iw[type]?>&ep=<?=$iw[store]?>&gp=<?=$iw[group]?>'">
							<i class="fa fa-undo"></i>
							목록
						</button>
					</div>
