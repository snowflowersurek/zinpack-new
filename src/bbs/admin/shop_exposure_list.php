						...
						<section class="content-box">
							...
							<div class="table-set-mobile dataTable-wrapper">
								...
								<table class="table table-striped table-bordered table-hover dataTable">
									...
									<tbody>
									<?
										$sql = "select * from $iw[shop_data_table] where ep_code = '$iw[store]' and gp_code = '$iw[group]' and sd_display in (0,1) $search_sql";
										...
										$sql = "select * from $iw[shop_data_table] where ep_code = '$iw[store]' and gp_code = '$iw[group]' and sd_display in (0,1) $search_sql order by sd_no desc limit $start_line, $end_line";
										$result = sql_query($sql);
									...
									?>
										<tr>
											<td data-title="상품코드"><a href="<?=$iw['admin_path']?>/shop_exposure_view.php?type=<?=$iw[type]?>&ep=<?=$iw[store]?>&gp=<?=$iw[group]?>&idx=<?=$sd_code?>"><?=$sd_code?></a></td>
											...
											<td data-title="노출"><a href="javascript:exposure_edit('<?=$iw[type]?>','<?=$iw[store]?>','<?=$iw[group]?>','<?=$sd_code?>','<?=$sd_value?>');"><span class="label label-sm label-<?if($dd_display==1){?>warning<?}else{?>success<?}?>">변경하기</span></a></td>
										</tr>
									<?
										$i++;
										}
										if($i==0) echo "<tr><td colspan='7' align='center'>관리할 상품이 없습니다.</td></tr>";
									?>
									</tbody>
								</table>
								<div class="row">
...
<script type="text/javascript">
	function exposure_edit(type,ep,gp,no,dis) {
		if (dis == 1) {
			confirm_text = "해당상품을 노출로 변경하시겠습니까?"
		}else{
			confirm_text = "해당상품을 숨김으로 변경하시겠습니까?"
		}
		if (confirm(confirm_text)) {
			location.href="shop_exposure_ok.php?type="+type+"&ep="+ep+"&gp="+gp+"&idx="+no+"&dis="+dis;
		}
	}
</script>
<?
