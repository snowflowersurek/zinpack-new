...
$sqlm = "select * from $iw[shop_seller_table] where ep_code = '$iw[store]' and mb_code = '$row[mb_code]'";
$rowm = sql_fetch($sqlm);
?>
			...
			<!-- PAGE CONTENT BEGINS -->
				<form class="form-horizontal no-input-detail">
					...
					<div class="form-group">
						...
						<div class="col-sm-11">
							<p class="col-xs-12 col-sm-8 form-control-static"><?=$rowm["mb_code"]?> ( <?=$rowm["ss_name"]?> / <?=$rowm["ss_tel"]?> )</p>
						</div>
					</div>
					...
					<div class="form-group">
						...
						<div class="col-sm-11">
							...
							<?php
								...
								$sy_max = number_format($row2["sy_max"]);
							?>
								[<?=$sy_code?>] <?=national_money($iw[language], $sy_price);?> - <?=$sy_max?> <?=national_language($iw[language],"a0215","개");?>
							</p>
						</div>
					</div>
					...
					<div class="form-group">
						...
						<div class="col-sm-11">
							...
							<?php
								$sql2 = "select * from $iw[shop_option_table] where sd_code = '$sd_code' order by so_no asc";
				...
				<div class="clearfix form-actions">
					<div class="col-md-offset-3 col-md-9">
						<button class="btn btn-primary" type="button" onclick="javascript:exposure_edit('<?=$iw[type]?>','<?=$iw[store]?>','<?=$iw[group]?>','<?=$sd_code?>','<?=$sd_value?>');">
							<i class="fa fa-check"></i>
							<?=$sd_btn?> 변경
						</button>
						<button class="btn btn-default" type="button" onclick="location='<?=$iw['admin_path']?>/shop_exposure_list.php?type=<?=$iw[type]?>&ep=<?=$iw[store]?>&gp=<?=$iw[group]?>'">
