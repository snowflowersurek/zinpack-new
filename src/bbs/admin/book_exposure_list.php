						...
						<section class="content-box">
							...
							<div class="table-set-mobile dataTable-wrapper">
								...
								<table class="table table-striped table-bordered table-hover dataTable">
									...
									<tbody>
									<?
										$sql = "select * from $iw[book_data_table] where ep_code = '$iw[store]' and gp_code = '$iw[group]' and bd_display <> 0 $search_sql";
										...
										$sql = "select * from $iw[book_data_table] where ep_code = '$iw[store]' and gp_code = '$iw[group]' and bd_display <> 0 $search_sql order by bd_no desc limit $start_line, $end_line";
										$result = sql_query($sql);
									...
									?>
										<tr>
											<td data-title="이북코드"><a href="<?=$iw['admin_path']?>/book_exposure_view.php?type=<?=$iw[type]?>&ep=<?=$iw[store]?>&gp=<?=$iw[group]?>&idx=<?=$bd_code?>"><?=$bd_code?></a></td>
											...
											<td data-title="노출"><a href="javascript:exposure_edit('<?=$iw[type]?>','<?=$iw[store]?>','<?=$iw[group]?>','<?=$bd_code?>','<?=$bd_value?>');"><span class="label label-sm label-<?if($bd_display==1){?>warning<?}else{?>success<?}?>">변경하기</span></a></td>
										</tr>
									<?
										$i++;
										}
										if($i==0) echo "<tr><td colspan='8' align='center'>관리할 이북이 없습니다.</td></tr>";
									?>
									</tbody>
								</table>
								<div class="row">
