						...
						<section class="content-box">
							...
							<div class="table-set-mobile dataTable-wrapper">
									...
									<div class="col-sm-6">
										<div class="dataTable-option-right">
											<?
												...
												if($search =="a"){
													$search_sql = "and ap_out_trade_no like '%$searchs%'";
												}else if($search =="b"){
													$search_sql = "and ap_trade_no like '%$searchs%'";
												}
											?>
											...
											<label>검색: <select name="search">
												...
												<option value="b" <?if($search == "b"){?>selected="selected"<?}?>>승인번호</option>
								...
								<table class="table table-striped table-bordered table-hover dataTable">
									...
									<tbody>
									<?
										$sql = "select * from $iw[alipay_table] where ep_code = '$iw[store]' and state_sort = '$iw[type]' $search_sql";
										...
										$sql = "select * from $iw[alipay_table] where ep_code = '$iw[store]' and state_sort = '$iw[type]' $search_sql order by ap_no desc limit $start_line, $end_line";
										$result = sql_query($sql);

										$i=0;
										while($row = @sql_fetch_array($result)){
											$lgd_oid = $row["ap_out_trade_no"];
											$lgd_amount = $row["ap_total_fee"];
											$lgd_financeauthnum = $row["ap_trade_no"];
											$lgd_respmsg = $row["ap_trade_status"];
											$lgd_paydate = date( "Y-m-d H:i:s", strtotime($row["ap_datetime"]) );
									?>
										<tr>
											<td data-title="주문번호"><a href="<?=$iw['admin_path']?>/shop_post_all_view.php?type=<?=$iw[type]?>&ep=<?=$iw[store]?>&gp=<?=$iw[group]?>&idx=<?=$lgd_oid?>"><?=$lgd_oid?></a></td>
											<td data-title="승인금액"><?=national_money("en", $lgd_amount);?></td>
											<td data-title="승인번호"><?=$lgd_financeauthnum?></td>
									...
									<?
										...
										if($i==0) echo "<tr><td colspan='5' align='center'>ALIPAY결제내역이 없습니다.</td></tr>";
									?>
									</tbody>
								</table>
								<div class="row">
