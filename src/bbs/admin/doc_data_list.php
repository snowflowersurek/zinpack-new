							...
							<div class="table-set-mobile dataTable-wrapper">
								...
								<table class="table table-striped table-bordered table-hover dataTable">
									...
									<?
										...
										$sql = "select * from $iw[doc_data_table] where ep_code = '$iw[store]' and gp_code = '$iw[group]' and mb_code='$iw[member]' $search_sql order by dd_no desc limit $start_line, $end_line";
										$result = sql_query($sql);
										...
										while($row = @sql_fetch_array($result)){
											...
											$dd_sell = number_format($row["dd_sell"]);
									...
									?>
										<tr>
											<td data-title="컨텐츠코드"><a href="<?=$iw['admin_path']?>/doc_data_view.php?type=<?=$iw[type]?>&ep=<?=$iw[store]?>&gp=<?=$iw[group]?>&idx=<?=$dd_code?>"><?=$dd_code?></a></td>
								...
								<div class="row">
									<div class="col-sm-6">
										<div class="dataTable-info">
											<button class="btn btn-success" type="button" onclick="location.href='<?=$iw['admin_path']?>/doc_data_write.php?type=<?=$iw[type]?>&ep=<?=$iw[store]?>&gp=<?=$iw[group]?>'">
