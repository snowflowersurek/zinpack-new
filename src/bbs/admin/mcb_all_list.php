						...
						<section class="content-box">
							...
							<div class="table-set-mobile dataTable-wrapper">
									...
									<div class="col-sm-6">
										<div class="dataTable-option-right">
											...
											<label>검색: <select name="search">
												<option value="a" <?php if{?>selected="selected"<?php }?>>게시글코드</option>
								...
								<table class="table table-striped table-bordered table-hover dataTable">
									...
									<tbody>
									<?php
										$sql = "select * from $iw[mcb_data_table] where ep_code = '$iw[store]' $search_sql";
										...
										$sql = "select * from $iw[mcb_data_table] where ep_code = '$iw[store]' $search_sql order by md_no desc limit $start_line, $end_line";
										$result = sql_query($sql);
										...
										while($row = @sql_fetch_array($result)){
											...
											$row2 = sql_fetch("select count(*) as cnt from $iw[comment_table] where ep_code = '$iw[store]' and gp_code='$gp_code' and state_sort = '$iw[type]' and cm_code = '$md_code' and cm_display = 1");
											$reply_count = number_format($row2[cnt]);
									?>
										<tr>
											<td data-title="게시글코드"><a href="<?=$iw['admin_path']?>/mcb_all_view.php?type=<?=$iw[type]?>&ep=<?=$iw[store]?>&gp=<?=$iw[group]?>&idx=<?=$md_code?>"><?=$md_code?></a></td>
											...
											<td data-title="링크">
												<a href="<?=$iw['m_path']?>/mcb_data_view.php?type=<?=$iw[type]?>&ep=<?=$iw[store]?>&gp=<?=$gp_code?>&item=<?=$md_code?>"><span class="label label-sm label-default">바로가기</span></a>
												<a href="<?=$iw['m_path']?>/mcb_data_edit.php?type=<?=$iw[type]?>&ep=<?=$iw[store]?>&gp=<?=$gp_code?>&item=<?=$md_code?>"><span class="label label-sm label-danger">글수정</span></a>
											</td>
										</tr>
									<?php
										$i++;
