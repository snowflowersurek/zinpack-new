						...
						<section class="content-box">
							...
							<div class="table-set-mobile dataTable-wrapper">
									...
									<div class="col-sm-6">
										<div class="dataTable-option-right">
											<?
												...
												if($search =="a"){
													$search_sql = "and pp_invoice like '%$searchs%'";
												}else if($search =="b"){
													$search_sql = "and pp_txn_id like '%$searchs%'";
												}
											?>
								...
								<table class="table table-striped table-bordered table-hover dataTable">
									...
									<tbody>
									<?
										$sql = "select * from $iw[paypal_table] where ep_code = '$iw[store]' and state_sort = '$iw[type]' $search_sql";
										...
										$sql = "select * from $iw[paypal_table] where ep_code = '$iw[store]' and state_sort = '$iw[type]' $search_sql order by pp_no desc limit $start_line, $end_line";
										$result = sql_query($sql);

										$i=0;
										while($row = @sql_fetch_array($result)){
											$lgd_oid = $row["pp_invoice"];
											$lgd_amount = $row["pp_mc_gross"];
											$lgd_financeauthnum = $row["pp_txn_id"];
											$lgd_respmsg = $row["pp_payment_status"];
											$lgd_paydate = date( "Y-m-d H:i:s", strtotime($row["pp_datetime"]) );
									?>
										<tr>
									...
									<?
										...
										if($i==0) echo "<tr><td colspan='5' align='center'>PAYPAL결제내역이 없습니다.</td></tr>";
									?>
									</tbody>
								</table>
								<div class="row">
