							...
							<div class="table-set-mobile dataTable-wrapper">
								...
								<table class="table table-striped table-bordered table-hover dataTable">
									...
									<?
										...
										$sql = "select * from $iw[shop_seller_table] where ep_code = '$iw[store]' $search_sql order by ss_no desc limit $start_line, $end_line";
										$result = sql_query($sql);
										...
										while($row = @sql_fetch_array($result)){
											...
											$ss_name = $row["ss_name"];
											$ss_tel = $row["ss_tel"];
											$ss_display = $row["ss_display"];
											$ss_datetime = date("Y.m.d", strtotime($row["ss_datetime"]));
									?>
										<tr>
											<td data-title="회원코드"><a href="<?=$iw['admin_path']?>/shop_seller_view.php?type=<?=$iw[type]?>&ep=<?=$iw[store]?>&gp=<?=$iw[group]?>&idx=<?=$ss_no?>"><?=$mb_code?></a></td>
