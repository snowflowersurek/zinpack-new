						...
						<section class="content-box">
							...
							<div class="table-set-mobile dataTable-wrapper">
								...
								<table class="table table-striped table-bordered table-hover dataTable">
									...
									<tbody>
									<?
										$sql = "select * from $iw[member_table] where ep_code = '$iw[store]' and mb_display = 3";
										...
										$sql = "select * from $iw[member_table] where ep_code = '$iw[store]' and mb_display = 3 order by mb_no desc limit $start_line, $end_line";
										$result = sql_query($sql);
										...
										<tr>
											...
											<td data-title="신청일"><?=$mb_datetime?></td>
											<td data-title="가입승인">
												<a href="javascript:approval_edit('<?=$iw[type]?>','<?=$iw[store]?>','<?=$iw[group]?>','<?=$mb_no?>');"><span class="label label-sm label-success">승인</span></a>
											</td>
										</tr>
									<?
										$i++;
...
<script type="text/javascript">
	function approval_edit(type,ep,gp,no) {
		if (confirm("회원가입을 승인하시겠습니까?")) {
			location.href="member_join_list_ok.php?type="+type+"&ep="+ep+"&gp="+gp+"&idx="+no;
		}
	}
</script>
