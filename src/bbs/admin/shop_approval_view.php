...
$sql = "select * from $iw[shop_data_table] where ep_code = '$iw[store]' and gp_code = '$iw[group]' and sd_code = '$sd_code'";
$row = sql_fetch($sql);
			...
			<!-- PAGE CONTENT BEGINS -->
				...
				<div class="clearfix form-actions">
					<div class="col-md-offset-3 col-md-9">
						<button class="btn btn-primary" type="button" onclick="javascript:approval_edit('<?=$iw[type]?>','<?=$iw[store]?>','<?=$iw[group]?>','<?=$sd_code?>','1');">
							<i class="fa fa-check"></i>
							등록승인
						</button>
						<button class="btn btn-default" type="button" onclick="location='<?=$iw['admin_path']?>/shop_approval_list.php?type=<?=$iw[type]?>&ep=<?=$iw[store]?>&gp=<?=$iw[group]?>'">
