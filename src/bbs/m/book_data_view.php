...
$row = sql_fetch($sql);
if (!$row["bd_no"]) alert(national_language($iw[language],"a0035","잘못된 접근입니다."),"");
...
$bd_recommend = $row["bd_recommend"];
$bd_datetime = date("Y-m-d", strtotime($row["bd_datetime"]));
...
$cg_level_download = $row2[cg_level_download];
$cg_url = $row2[cg_url];
...
if ($row2[cg_level_read] != 10 && $iw[level]=="guest"){
	...
	alert(national_language($iw[language],"a0169","해당 카테고리에 읽기 권한이 없습니다."),$iw[m_path]."/book_data_list.php?type=".$iw[type]."&ep=".$iw[store]."&gp=".$iw[group]."&menu=".$cg_code_view);
}

$kakaoContent = strip_tags($row["bd_content"]);
...
if($row["bd_image"]){
	$kakaoImg = $iw[url].$upload_path."/".$bd_code."/".$row["bd_image"];
}
...
<div class="content">
	<div class="row">
		...
		<div class="grid-sizer"></div>
			...
			<div class="masonry-item w-4 h-full">
				<div class="box box-media br-theme">
					<?php if($bd_image){?>
						<img class="media-object img-responsive" src="<?=$upload_path?>/<?=$bd_code?>/<?=$bd_image?>" alt="">
					<?php }?>
				</div> <!-- /.box -->
			</div> <!-- /.masonry-item -->
			...
			<div class="masonry-item w-8 h-full">
				<div class="box br-theme">
					<h3 class="media-heading"><?=$bd_subject?></h3>
					<br>
					<div class="content-info">
						<ul class="list-inline">
							<li><i class="fa fa-file"></i> <?=$bd_style?></li>
							<?php if($cg_date==1){?><li><i class="fa fa-calendar"></i> <?=$bd_datetime?></li><?php } ?><?php if($cg_writer==1){?><li><i class="fa fa-user"></i> <?=$mb_nick?></li><?php } ?><?php if($cg_hit==1){?><li><i class="fa fa-eye"></i> <?=$bd_hit?></li><?php } ?><?php if($cg_comment_view==1){?><li><i class="fa fa-comment"></i> <?=$reply_count?></li><?php } ?><?php if($cg_recommend_view==1){?><li><i class="fa fa-thumbs-up"></i> <span id="recommend_board_1"><?=$bd_recommend?></span></li><?php }?>
						</ul>
						<ul class="list-inline">
							<li><strong>저자 : </strong><span class="label label-info"><?=$bd_author?></span></li>
							<li><strong>출판사 : </strong><span class="label label-info"><?=$bd_publisher?></span></li>
							<li><strong><?=national_language($iw[language],"a0151","자료가격");?> : </strong><span class="label label-info"><?php if{?><?=national_language($iw[language],"a0265","무료");?><?php }else{?><?=$bd_price?> Point<?php }?></span></li>
						</ul>
					</div>
					<div class="col-md-6">
					...
					<div class="col-md-6">
						<div class="pull-right">
							<?php if($cg_level_download == 10 || $iw[mb_level] >= $cg_level_download || $mb_code == $iw[member]){?>
								<a class="btn btn-theme" href="javascript:win_viewer('<?=$iw[type]?>', '<?=$iw[store]?>', '<?=$iw[group]?>', '<?=$bd_code?>');">샘플보기</a>
							<?php }?>
							<a href="javascript:downloadBook('book_data_download.php?type=<?=$iw[type]?>&ep=<?=$iw[store]?>&gp=<?=$iw[group]?>&item=<?=$bd_code?>')" class="btn btn-theme"><?=national_language($iw[language],"a0227","바로 구매하기");?></a>
						</div>
					</div>
			...
			<div class="masonry-item w-<?=$hm_view_size?> h-full">
				<div class="box br-default">
					<div class='image-container'><?=$bd_content?></div>
				</div> <!-- /.box -->
			</div> <!-- /.masonry-item -->
		...
		<!-- / 베스트댓글 리스트 -->
		<?php
			$sql2 = "select * from $iw[comment_table] where ep_code = '$iw[store]' and gp_code='$iw[group]' and state_sort = '$iw[type]' and cm_code = '$bd_code' and cm_display = 1 and cm_recomment = 0 and cm_recommend > 2 order by cm_recommend desc limit 0, 3";
		...
		?>
			<div id="best_comment_<?=$i?>" class="masonry-item w-full h-full">
				<div class="box br-theme">
					...
					<?php if((($iw[gp_level] != "gp_guest" && $iw[group] != "all") || ($iw[level] != "guest" && $iw[group] == "all")) && ($iw[mb_level] >= $row2[cg_level_comment])){?>
		...
		<?php if((($iw[gp_level] != "gp_guest" && $iw[group] != "all") || ($iw[level] != "guest" && $iw[group] == "all")) && ($iw[mb_level] >= $row2[cg_level_comment])){?>
			...
			<div class="masonry-item w-full h-full">
				<div class="box br-theme">
					<input type="hidden" name="cm_code" value="<?=$bd_code?>" />
		...
		<!-- / 댓글 리스트 -->
		<?php
			$sql2 = "select * from $iw[comment_table] where ep_code = '$iw[store]' and gp_code='$iw[group]' and state_sort = '$iw[type]' and cm_code = '$bd_code' and cm_display = 1 and cm_recomment = 0 order by cm_no desc";
			$result2 = sql_query($sql2);
				...
				<div class="box br-theme">
					<h4 class="media-heading box-title">
						...
						<?php if($row2["mb_code"] == $iw[member]){?>
							<a href="javascript:comment_delete('<?=$iw[type]?>','<?=$iw[store]?>','<?=$iw[group]?>','<?=$bd_code?>','<?=$cm_no?>');" class="text-danger"><i class="fa fa-times-circle"></i></a>
					...
					<?php }
						$sql4 = "select * from $iw[comment_table] where ep_code = '$iw[store]' and gp_code='$iw[group]' and state_sort = '$iw[type]' and cm_code = '$bd_code' and cm_display = 1 and cm_recomment = '$cm_no' order by cm_no asc";
...
<script type="text/javascript">
	...
	function win_viewer(type, ep, gp, idx)
    {
...
$sql = "update $iw[book_data_table] set
		bd_hit = bd_hit+1
		where ep_code = '$iw[store]' and gp_code='$iw[group]' and bd_display = 1 and bd_code = '$bd_code'";
sql_query($sql);
