...
if ($verified) {
	...
	$sql = "insert into $iw[paypal_table] set
			...
			pp_datetime = '$pt_datetime',
			pp_display = 0
			";
	sql_query($sql);

	if ($PAYPAL_payment_status=='Completed') {
		$row = sql_fetch(" select count(*) as cnt from $iw[paypal_table] where ep_code = '$iw[store]' and state_sort = '$iw[type]' and mb_code = '$PAYPAL_custom' and pp_invoice = '$PAYPAL_invoice' and pp_display = 1");
		if (!$row[cnt]) {
				//최종결제요청 결과 성공 DB처리
			...
			$sql = "update $iw[member_table] set
					mb_point = mb_point+$PAYPAL_item_name
