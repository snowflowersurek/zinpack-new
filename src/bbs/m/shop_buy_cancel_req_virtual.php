...
array(3) { ["re_bank"]=> string(7) "SHINHAN" ["re_account"]=> string(13) "4534634634534" ["re_name"]=> string(9) "홍길동" }
위 값을 저장할 테이블 : 

array(4) { ["type"]=> string(4) "shop" ["ep"]=> string(25) "ep1822322763609cab5915c89" ["gp"]=> string(3) "all" ["item"]=> string(16) "8874790022290516" }
*/
if ($iw[level]=="guest") alert(national_language($iw[language],"a0003","로그인 해주시기 바랍니다."),"$iw[m_path]/all_login.php?type=$iw[type]&ep=$iw[store]&gp=$iw[group]");
	...
	$insql = "INSERT INTO $iw[lgd_cancel_table] SET 
				ep_code		= '$iw[store]',
				mb_code		= '$iw[member]',
				state_sort	= '$iw[type]',
				lgd_oid		= '$sr_code',
...
?>
<body>
	<form method="post" name ="LGD_PAYINFO" id="LGD_PAYINFO" action="http://<?=$iw[pay_site]?>/_payment/toss_payments_virtual_cancel.php?type=<?=$iw[type]?>&ep=<?=$iw[store]?>&gp=<?=$iw[group]?>">
		...
		<input type="hidden" name="RE_BANK"						id="RE_BANK"			value="<?= $RE_BANK ?>">
		<input type="hidden" name="RE_ACCOUNT"					id="RE_ACCOUNT"			value="<?= $RE_ACCOUNT ?>">
